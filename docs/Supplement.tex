...
\date{Software version 1.2.3 \quad\textbar\quad Document compiled \today}
...
\midrule
Package version           & 1.2.3              & SemVer; the audit trail
...
\paragraph{Grid extent (v1.2.2).}
Context modes anchor the interpolation; they do not decide how far the
grid reaches. That is settled by \fn{export.interp\_grid\_bounds}, which
since v1.2.2 reproduces the requested range exactly: the grid runs from
\cfgkey{freq\_min} to \cfgkey{freq\_max} (in multi-window mode, from the
hull of all windows for the top-level overall export), with a missing
lower bound taken as $0.0$ -- the same convention as
\fn{Config.get\_windows} and the \texttt{0--800\_cm-1} folder label.
Grid points with no mode nearby take the \texttt{np.interp}
\texttt{left}/\texttt{right}~$= 0.0$ value, which reads as ``no modes in
this range''. Only a run with no requested range at all
(\cfgkey{freq\_min}, \cfgkey{freq\_max} and \cfgkey{freq\_windows} all
unset) follows the data, padded by \cfgkey{interp\_edge\_extend}. Before
v1.2.2 the data-driven fallback also applied whenever
\cfgkey{freq\_min}/\cfgkey{freq\_max} were internally cleared -- which is
exactly what the multi-window overall export does so that
\fn{Config.outdir} creates no frequency subfolder -- so a request for
0--100 cm$^{-1}$ produced a top-level grid starting at the first real
mode while the \texttt{0--100\_cm-1} subfolder started at $0.00$.

\paragraph{Multi-window context (v1.2.3).}
In multi-window mode the runner analyses all modes inside the hull of
\cfgkey{freq\_windows} and then exports once per window plus one
overall export. Up to v1.2.2 two things went wrong at the hull edges.
First, the context modes were only loaded when \cfgkey{freq\_max}
(\cfgkey{freq\_min}) happened to be set -- fields that are documented
as ignored in multi-window mode -- so a TOML with only
\cfgkey{freq\_windows} loaded none. Second, the per-window export took
its context candidates from the already hull-filtered result list, so
even when the runner had analysed them (``\texttt{97 context modes
right}'') the top window ended without a right anchor and emitted
``\texttt{interp\_boundary\_mode='context' but no context modes
available}''; \texttt{np.interp} then fell to \texttt{right}~$=0$
between the last analysed mode and the window bound. Since v1.2.3 the
context ranges are derived from the hull ($\min lo$, $\max hi$; an
open upper window loads no right context), the candidate pool of
\fn{runner.\_window\_context} is the union of the analysed modes and
both context lists, and the overall export receives the context lists
as well.

\paragraph{Locked output files (v1.2.3).}
All Excel exporters save through \fn{export.\_save\_workbook}. A
\texttt{PermissionError} on the target -- on Windows almost always the
file of the previous run still open in Excel -- no longer drops the
workbook: it is written to \texttt{<name>\_1.xlsx} (\texttt{\_2},
\ldots) and the fallback is printed on the console and recorded as a
REPORT warning. The motivating case was a multi-window run whose
interpolated Excel ``looked un-interpolated'': the REPORT carried
``\texttt{interpolation Excel failed: [Errno 13] Permission denied}''
and the file being inspected was the stale one of an earlier run.
...
\cfgkey{interp\_boundary\_mode} & \texttt{str} & \texttt{"context"}
  & Boundary handling: \texttt{"context"} loads context modes
    ...
    the window, \texttt{"nearest"} flatly extends the boundary
    value. Any other value is rejected by \fn{Config.validate}. \\
\cfgkey{interp\_edge\_extend} & \texttt{float} & \texttt{0.5}
  & Grid padding (cm$^{-1}$) beyond the first/last mode. Since
    v1.2.2 in effect \emph{only} when no frequency range is
    requested; with a range the grid spans exactly that range
...
This part documents the test suite. As of v1.2.3 it comprises
201 unit tests + 4 end-to-end (E2E) smoke tests (205 total; the
E2E tests carry the \texttt{slow} marker and are deselected by
default). The detailed per-suite sections that follow describe the
v1.0.4 regression baseline; the suite was expanded in v1.1.0
(SSE-rename guards), v1.2.0 (critical-review regression tests,
including a default-run headline-$\Lambda$ regression and PCET
angle-criterion tests), v1.2.1 (ORCA reduced-mass reconstruction),
v1.2.2 (interpolation grid range) and v1.2.3 (export robustness).

% -------------------------------------------------------------------------
\section{Overview: 205 tests}

\begin{longtable}{p{0.34\textwidth} p{0.08\textwidth} p{0.50\textwidth}}
\toprule
\textbf{Suite} & \textbf{Tests} & \textbf{Purpose} \\
\midrule
\fn{test\_config.py} & 26 & Config defaults, TOML parsing/coercion,
    validation, field inventory. \\
\fn{test\_v104\_audit\_fixes.py} & 24
  & Regression guards for the FUND findings, half-open windows,
    plus SCSD sigma-scaling and symmetry-validation tests. \\
\fn{test\_core\_physics.py} & 23 & Thermal amplitude, OOP/INP,
    kernel metrics, classification. \\
\fn{test\_sanity\_physics.py} & 19
  & Physical property tests on $\urms$, OOP/INP, $\Delta r$,
    $\lambda$, RSS, and the HA reaction coordinate -- run against the
    implementation without requiring a Gaussian log. \\
\fn{test\_reorganization.py} & 18 & Marcus--Hush $\lambda$ per channel,
    additivity, scaling, non-negativity. \\
\fn{test\_pcet\_et.py} & 14 & H-bond acceptor detection, exclusions,
    element filter, and the D--H$\cdots$A angle criterion. \\
\fn{test\_interp\_grid\_range\_v122.py} & 14 & Interpolation grid spans
    the requested range: explicit bounds, upper-bound-only $\to 0.0$,
    no-range runs stay data-driven, infinite/degenerate bounds fall back
    safely, exporter keyword parity. \\
\fn{test\_v123\_export\_robustness.py} & 10 & Locked-file fallback of
    \fn{export.\_save\_workbook}, \fn{runner.\_window\_context}
    selection, REPORT base directory in multi-window mode. \\
\fn{test\_ca\_umap\_and\_exports.py} & 10 & C$\alpha$-UMAP embedding and
    Excel/plot export paths. \\
\fn{test\_sse\_physics\_v105.py} & 8 & SSE rigid-body decomposition
    (translation/rotation/internal) against ground truth. \\
\fn{test\_multi\_cluster.py} & 7 & Multi-[2Fe-2S] detection and the
    per-cluster wrapper. \\
\fn{test\_group\_map\_indexing.py}, \fn{test\_manual\_code\_refs.py}
  & 12 & Group/center mapping; every \codref{} points to a real function. \\
\fn{test\_regression\_reorg.py} & 6 & Default-run headline regression
    pinning $\Lambda_\text{FeFe}/\Lambda_\text{FeS}$ for the reference
    fixture ($\pm 2\,\%$). \\
\fn{test\_helpers\_v35.py} & 5 & Bend-split invariants and NaN guards. \\
\fn{test\_v110\_rename.py} & 3 & SS$\to$SSE rename / legacy-alias guards. \\
\fn{test\_orca\_reduced\_mass.py} & 2 & ORCA reduced-mass reconstruction
    $\mu_k = \sum_i m_i|l_{i,k}|^2$ (incl.\ analytic pure-translation case). \\
\fn{test\_smoke.py} (E2E, \texttt{slow}) & 4 & End-to-end smoke tests on
    the Cys$_4$ benchmark log (full pipeline to Excel). \\
\midrule
