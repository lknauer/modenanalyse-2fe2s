...
\date{Software version 1.0.4 \quad\textbar\quad Document compiled \today}
...
\begin{itemize}
  ...
  \item \emph{Marcus-Hush reorganization}: per-mode
        ...
        $\Lambda_X(\omega)$.
  \item \emph{Secondary-structure} descriptors: per-SS-element
  ...
  \item \emph{Embeddings}: three parallel UMAP-HDBSCAN clusterings on
        ...
        features, SS-element fingerprint, C$_\alpha$-amplitude
        fingerprint).
...
\midrule
Package version           & 1.0.4              & SemVer; the audit trail
...
(bond modulation, bend, group-amplitude, SCSD amplitude, SS-element
...
\subsection{Geometric weighting by H$\cdots$A distance}
...
\codref{geometry.detect\_ss\_dssp, geometry.detect\_ss\_phipsi}

Two parallel secondary-structure (SS) classifiers are implemented.
...
\begin{lstlisting}[style=pseudo,
  caption={DSSP-record extraction from PDB. Implementation:
           \protect\fn{geometry.detect\_ss\_dssp}.}]
input:  pdb_path
output: ss_per_residue : dict[(chain, resnum) -> ss_type]
    ...
    foreach line in f:
        if line starts with "HELIX":
            ...
            foreach r in range(resnum_a, resnum_b + 1):
                ss_per_residue[(chain, r)] <- "H"
        if line starts with "SHEET":
            chain     <- line[21]
            resnum_a  <- parse_int(line[22:26])
            resnum_b  <- parse_int(line[33:37])
            foreach r in range(resnum_a, resnum_b + 1):
                ss_per_residue[(chain, r)] <- "E"

return ss_per_residue
\end{lstlisting}

\subsection{Algorithm: $\phi/\psi$-only classifier}

\codref{geometry.detect\_ss\_phipsi}

If no SS records are in the PDB and DSSP is not installed, the
...
\begin{lstlisting}[style=pseudo,
  caption={$\phi/\psi$ secondary-structure classifier.
           Implementation: \protect\fn{geometry.detect\_ss\_phipsi}.}]
...
foreach residue (chain, resnum) in pdb_atoms:
    ...
    raw_ss[(chain, resnum)] <- classify(phi, psi)

# Apply min-length filter: drop isolated SS-classified residues
ss_per_residue <- min_length_filter(raw_ss, alpha_min=3, beta_min=2)
return ss_per_residue
\end{lstlisting}
...
downstream SS-amplitude analysis; the choice is logged.

\subsection{The fallback cascade}

The full SS-detection pipeline tries three sources in order, stopping
...
``\texttt{ss\_source = pdb\_records}'', ``\texttt{ss\_source =
dssp\_executable}'', or ``\texttt{ss\_source = phipsi\_fallback}''.

\subsection{Edge cases}

\paragraph{(S1) PDB with no SS records and no DSSP.}
...
\cfgkey{ss\_alpha\_min\_len} and \cfgkey{ss\_beta\_min\_len}.
...
\subsection{Connection to the SS-amplitude analysis}

The output \texttt{ss\_per\_residue} dict feeds two downstream
consumers:

\begin{enumerate}
  \item \texttt{export\_ss\_amplitude\_groups}: builds per-mode
        SS-aggregated amplitude sheets. Residues classified
  ...
  \item \fn{geometry.build\_ss\_center\_map}: enriches the
        per-atom residue labels with their SS class for the audit
        log.
\end{enumerate}

For both, an unclassified residue (UNK) is silently dropped from
the SS-sum but kept in the total -- the resulting ``$\alpha + \beta
+ \text{loop} < \text{total}$'' inequality in the workbook signals
to the user that some residues lacked SS classification.
...
$B$-factor and SS-amplitude loops detect the sentinel and skip the
...
\texttt{ss\_amplitude\_*} & \texttt{float}
  & Per-SS-element amplitudes (only when SS analysis is enabled). \\
\bottomrule
\end{longtable}
...
\cfgkey{analyze\_ss} & \texttt{bool} & \texttt{True}
...
\cfgkey{ss\_chain} & \texttt{str} & \texttt{""}
  & Restrict SS analysis to a single chain. Empty = all chains. \\
\bottomrule
\end{longtable}
...
% -------------------------------------------------------------------------
\section{SS workbook (\file{*\_analysis\_SS.xlsx})}

\codref{export.export\_ss\_excel}

Produced only when SS analysis is enabled and a PDB with SS records
(or DSSP records) is supplied. Sheets enumerate per-SS-element
amplitudes per mode, plus axial-tilt diagnostics.

\subsection*{Sheets}

\begin{itemize}
  \item \texttt{SS\_amplitude\_mean}: per-mode, per-SS-element mean
        atom-displacement amplitude (\AA{}).
  \item \texttt{SS\_amplitude\_max}: per-mode, per-SS-element maximum
        atom-displacement amplitude.
  \item \texttt{SS\_com\_amplitude}: per-mode, per-SS-element
        centre-of-mass displacement.
  \item \texttt{SS\_axial\_amplitude}: axial component of the
        SS-element centre-of-mass displacement.
  \item \texttt{SS\_tilt\_angle\_deg}: rotation angle (deg) of the
        SS-element's principal axis under the mode.
  \item \texttt{SS\_elements}: index sheet with per-element residue
        ranges and chain assignments.
\end{itemize}
...
\begin{itemize}
  ...
  \item \emph{SS-fingerprint}: per-mode SS-element amplitudes
        (only when SS is enabled).
...
applied to the SS-amplitude and group-amplitude loops.
...
\midrule
\texttt{test\_ss\_element\_sigma\_responds\_to\_sigma\_eigvec}
 & Construct a small SS-element with 3 atoms, run the export-
   amplitude function with \cfgkey{sigma\_eigvec} = $10^{-4}$ and
   then $5\!\times\!10^{-4}$. Assert sigma ratio = 5.
 & FUND 7 -- SS-element sigmas should be user-configurable. \\
\midrule
...
The DSSP algorithm referenced in \cref{sec:algo-ss}.
...
The $\phi/\psi$ classification used in the SS-detection fallback.
...
$\OOP, \INP$ & Out-of-plane / in-plane component, relative to
                $\nhat$. \\
$G$ & Atom group (e.g.\ cluster, residue, SS-element). \\
\bottomrule
\end{longtable}
...
\texttt{export.export\_ss\_excel}                     & \texttt{export.py}         & 593--644  \\
\texttt{geometry.cluster\_normal}                     & \texttt{geometry.py}       & 336--371  \\
\texttt{geometry.detect\_ss\_dssp}                    & \texttt{geometry.py}       & 1240--1422 \\
\texttt{geometry.detect\_ss\_phipsi}                  & \texttt{geometry.py}       & 1423--1566 \\
