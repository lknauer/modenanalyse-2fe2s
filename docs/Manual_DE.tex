...
% --- Zusaetzliche Macros aus dem nisspec3-Handbuch (Teil III) -----------
...
\texttt{nisspec3} ist ein eigenst\"andiges Python-Paket f\"ur die
Simulation reiner NIS-Spektren und der zugeh\"origen Fe-Thermodynamik
(Maradudin-Bessel-Mehrphonon-Entwicklung, ina32-kompatibel). Es ist
allgemein f\"ur jedes M\"o\ss bauer-Isotop einsetzbar.

\textbf{Wichtiger Architektur-Wechsel ab v3.4}: \texttt{modenanalyse\_2fe2s}
hat \emph{keine} interne NIS-Berechnung mehr. NIS-Spektren, Fe-pDOS,
Lamb-Mössbauer-Faktor und Thermodynamik werden durch \texttt{nisspec3}
\emph{separat} bereitgestellt; das Modenanalyse-Werkzeug fokussiert
sich vollst\"andig auf die strukturelle Mode-Klassifikation.
...
\begin{enumerate}
  \item \textbf{NIS-Spektrum} (Maradudin-Bessel, multi-temperatur,
        ...
        \texttt{<input>.log}} oder \texttt{<input>.hess}.
...
Die beiden Auswertungen sind \emph{unabh\"angig}: identische Konstanten,
identische Eingabedatei, kein Datenaustausch zwischen den Tools n\"otig.
Damit l\"asst sich \texttt{nisspec3} unabh\"angig aktualisieren, ohne
\texttt{modenanalyse\_2fe2s} zu beeintraechtigen.

\subsubsection*{Was \texttt{modenanalyse\_2fe2s} eigenst\"andig liefert}
...
\begin{itemize}
  ...
  \item \textbf{Multi-Cluster-Unterst\"utzung}: Erkennung mehrerer
        {[}2Fe-2S{]}-Cluster im selben System (z.\,B.\ mitoNEET-Dimer)
        und Auswahl per Konfigurationsoption.
  \item \textbf{Embeddings} (UMAP, HDBSCAN-Clustering):
        zweidimensionale Projektion des Mode-Featureraums.
  \item \textbf{ORCA-Support} (eigenst\"andiger \texttt{.hess}-Parser):
        Mode-Analyse direkt aus ORCA-Frequenzrechnungen, ohne dass
        \texttt{nisspec3} ben\"otigt wird.
\end{itemize}
...
\begin{itemize}
  \item Python $\geq$ 3.11 mit \texttt{numpy}, \texttt{scipy},
        ...
        Wheel installiert.
  \item \texttt{nisspec3} ist \textbf{nicht} mehr erforderlich (siehe
        Abgrenzung oben).
...
\begin{itemize}
  ...
  \item \textbf{Element-Selektivit\"at}: Alle thermodynamischen
        ...
        $c_V$, $S$, $\langle k\rangle$, ...) sind aus der
        \textbf{$^{57}$Fe-projizierten} pDOS abgeleitet. Sie
        beschreiben den Mittelwert pro Fe-Atom, nicht den Cluster, das
        Protein oder das Gesamtsystem (vgl.\ \texttt{nisspec3}-Handbuch \cite{NisspecHandbuch}).
\end{itemize}
...
\begin{warnbox}
\textbf{Bugfix B3 (v3.0).} In Versionen vor v3.0 fehlte der Faktor
$1/2$ unter der Wurzel in Gl.~(\ref{eq:urms}). Das f\"uhrte zu
$\urms$-Werten, die um $\sqrt{2}$ \"uberh\"oht waren, und entsprechend
zu \"uberh\"ohten thermischen Auslenkungen in allen
geometrischen Scores. Resultate aus v$\leq$2.x sind in diesem
Punkt nicht ohne Korrektur vergleichbar.
\end{warnbox}
...
Da die Eigenvektoren mass-gewichtet sind, ist
$\alpha_{\OOP}$ zugleich der \emph{kinetische-Energie-Anteil} der Mode,
der senkrecht zur Cluster-Ebene flie\ss t -- dieser Bezug ist wichtig
f\"ur die sp\"atere Verbindung zur NRVS-Polarisationsabh\"angigkeit.
...
In v3.0 wurde zus\"atzlich eine \emph{feine Klassifikation} mit drei
...
In v3.0 ersetzt eine rigorose Funktion
(\texttt{core.extract\_dominant\_scsd\_irreps}) die fr\"uhere
...
gefolgt von HDBSCAN-Clustering.

PCA und t-SNE wurden in v3.7.2 entfernt; die Begr\"undung folgt im
Abschnitt \emph{Wahl der Methode}. Wer die alten Embeddings
ben\"otigt, findet sie in \texttt{modenanalyse\_2fe2s} v3.7.1.

\subsubsection*{Wahl der Methode: warum UMAP?}

Drei Methoden waren in v3.0--v3.7.1 parallel implementiert: PCA, t-SNE,
UMAP. Nach mehreren Test-L\"aufen auf protonierten und deprotonierten
Cys$_2$His$_2$-Modellsystemen sowie einer
HHCC-Mutante hat sich UMAP als die einzige Methode erwiesen, die f\"ur
DFT-Mode-Daten konsistent interpretierbare Ergebnisse liefert.
...
in v3.7.2) wird f\"ur jeden Datenpunkt $x_i$ ein lokaler
...
Pro Mode werden 31 numerische Features extrahiert. Die Auswahl ist
das Ergebnis einer schrittweisen Bereinigung (v3.7.2): in fr\"uheren
Versionen waren bis zu 33 Features eingebaut, von denen zwei
\texttt{Cys *\_OOP} per geometrischer Konstruktion immer null sind
(Cys-S$_\gamma$ liegt in der Cluster-Ebene, hat keine
Out-of-plane-Komponente). Diese wurden entfernt. Die verbleibenden 31
Features sind:
...
\begin{itemize}
  ...
  \item \emph{Sekund\"arstruktur} (HELIX/SHEET-Anteile): wird im
        ...
        SS-UMAP sind konzeptionell unterschiedliche Auswertungen.
\end{itemize}
...
\]
seit v3.1. F\"ur typische [2Fe-2S]-Protein-Systeme ($N \approx 5\,000$--$10\,000$) ergibt das
$\texttt{mcs} = 30$.
...
\texttt{modenanalyse\_2fe2s} delegiert die NIS-Spektrum-Simulation und
die Berechnung aller thermodynamischen Gr\"o\ss en
($\fLM$, $\langle x^2\rangle$, $c_V$, $S$, $\langle k\rangle$,
$\ER$) vollst\"andig an \texttt{nisspec3}.

Die theoretischen Grundlagen -- 1-Phonon-Absorption,
Maradudin-Bessel-Mehrphonon-Entwicklung, Pulvermittel,
Lamb-M\"o\ss bauer-Faktor, ina32-kompatible Thermo-Auswertung -- sind
in Teil~\ref{part:nis-theorie} (NIS- und Thermodynamik-Theorie)
ausf\"uhrlich dargestellt. Dieser Teil ist eine \emph{1:1-\"Ubernahme}
des nisspec3-Handbuchs \cite{NisspecHandbuch}, weil sich die Theorie
nicht zwischen den beiden Programmen unterscheidet (sie teilen sich
denselben Backend-Code).

In \texttt{modenanalyse\_2fe2s} wird zus\"atzlich:
\begin{itemize}
  \item Der \texttt{ParseResult} f\"ur die nisspec3-Maradudin-
        Entwicklung wird seit v3.2 \emph{in-memory} aus den
        Phase-4-Daten gebaut, statt die Logdatei ein zweites Mal zu
        lesen. Bei sehr gro\ss en Files (z.\,B.\ 10~GB-Logfiles bei sehr gro\ss en Cys$_2$His$_2$-Modellen)
        spart das ca.\ 1--2~Min I/O und ca.\ 5~GB Spitzen-RAM.
        Speicher-Schutz-Schwelle: \texttt{cfg.nis\_inmemory\_max\_atom\_modes}
        (Default $5\cdot 10^7$).
  \item Die NIS-Berechnung erfolgt einmal global \"uber alle
        analysierten Modes -- nicht pro Frequenzfenster --, weil die
        Mehrphonon-Faltung die volle pDOS ben\"otigt.
  \item Die berechnete Fe-pDOS wird per
        \texttt{nis.check\_pdos\_consistency} auf die Normierung
        $\int g_\Fei(\omega)\,d\omega = 1$ gepr\"uft (Toleranz
        $10^{-3}$). Abweichungen landen im Befund.
\end{itemize}
...
Die Multi-Mode-Sicht ist auch der Grund, warum die fr\"ueheren v3.6
...
Verteilung ist. Die mode-aufgel\"oste Reorg-Pipeline in v3.7 macht
...
Das Tool berechnet \emph{nicht} NRVS-Spektren -- das macht
\texttt{nisspec3} (siehe \S\ref{sec:nisspec-link}). Aber die in
...
\subsection{Mode-aufgel\"oste Bindungs-Reorganisations-Energien (v3.7)}
\label{sec:reorg}

In v3.7.0 hat \texttt{modenanalyse\_2fe2s} einen \emph{konzeptionellen
...
\end{equation}
wobei $\alpha_X(i) \in [0, 1]$ der Anteil der Mode auf der Bindung $X$
ist. Damit hat eine reine Streckmode der Bindung ($\alpha = 1$) bei
$T \to 0$ einen Reorg-Beitrag von $\hbar\omega/4$ (Nullpunkts-Energie
geteilt durch 2).
...
In v3.7.1 wurde dies korrigiert. Die HA-Modulation misst die
...
Seit v3.7.4 wird das Gauss-Gewicht
...
In v3.7.1--v3.7.3 wurde die Lambda-Aggregation mit $w$ multipliziert.
Dadurch waren Lambda$_{HA}$-Werte bei Akzeptoren mit
$r_{\text{eq}} > 2.8$\,\AA{} systematisch zu klein. Bei v3.7.4-L\"aufen
ist Lambda$_{HA}$ entsprechend gr\"o{\ss}er als bei v3.7.1--v3.7.3-L\"aufen
desselben Systems.
...
In v3.7 werden beide ausgegeben (Spalten \texttt{lambda\_X\_pair\_cm1}
...
\begin{itemize}
  \item \textbf{Modulations-Spektren} $M_X(\omega)$ -- Gauss-
        verbreiterte Akkumulation der absoluten Modulationen:
        \begin{equation}
          M_X(\omega)
...
\subsubsection*{Was im Code NICHT (mehr) drin ist}

Das v3.6-Kapitel \emph{PCET- und ET-Multi-Feature-Score} mit
$P_\text{CPET}$, $P_\text{PT}$, $P_\text{ET}$, der drei-Komponenten-
Konstruktion (Mode-Modulation $\times$ Lokalisierung $\times$
Cluster-Kopplung), $\tanh$-S\"attigung und Klassifikationsschwellen
"`stark"' / "`moderat"' wurde in v3.7.0 vollst\"andig entfernt. Die
...
werden seit v3.7.3 ignoriert (mit Warnung), damit alte TOMLs nicht
crashen. Sheets \texttt{CPET\_Kandidaten}, \texttt{PT\_Kandidaten},
\texttt{ET\_Kandidaten}, \texttt{CPET\_Pfade} existieren nicht mehr.

\paragraph{Wissenschaftliche Begr\"undung des Pivots.}
Die fr\"uhere Score-Konstruktion war ein "`Composite''-Score, der
versuchte, drei Eigenschaften (Modulation, Lokalisation,
Kopplungsst\"arke) in eine einzige Zahl pro Mode zu pressen. Die
Schwellen "`stark $> 0.5$"', "`moderat $> 0.2$"' hatten jedoch keinen
physikalischen Bezug -- sie waren empirisch so kalibriert, dass eine
"`typische CPET-Mode"' $\sim 0.5$ erreicht. Das ist
system-abh\"angig (nicht zwischen Strukturen vergleichbar) und lenkt
vom eigentlichen Befund ab: \emph{Frequenzbereiche}, in denen viele
Modes simultan mehrere Bindungen modulieren, sind bedeutsamer als
einzelne "`Top-Modes"'. Marcus-Hush-Reorg ist konzeptuell sauberer,
zwischen Strukturen vergleichbar (Werte in cm$^{-1}$), und liefert
direkt die Gr\"o{\ss}e, die in der Marcus-Rate eingeht.
...
\part{NIS- und Thermodynamik-Theorie -- siehe \texttt{nisspec3}}
\label{part:nis-theorie}
% =========================================================================

\begin{infobox}
\textbf{Wichtige \"Anderung in v3.4 (Mai 2026):}
Die Berechnung von NIS-Spektren, Fe-pDOS, Lamb-M\"o\ss bauer-Faktoren
und thermodynamischen Parametern wurde aus \texttt{modenanalyse\_2fe2s}
\textbf{entfernt}. \texttt{modenanalyse\_2fe2s} fokussiert sich jetzt
ausschlie\ss lich auf die \emph{Mode-Klassifikation} der schweren Atome
([2Fe-2S]-Cluster und Liganden): OOP/INP-Charakter, SCSD-Symmetrie-
Zerlegung, Kern-Scores, Marcus-Hush-Reorganisationsenergien (v3.7),
UMAP-Embedding mit HDBSCAN-Clustering. Diese Aufspaltung folgte der
Beobachtung, dass beide Schritte
(Mode-Klassifikation und NIS-Spektren-Synthese) auf derselben Logdatei
ablaufen, aber unabh\"angig voneinander n\"utzlich sind.

Wer NIS-Spektren oder Fe-pDOS oder Lamb-M\"o\ss bauer-Faktoren ben\"otigt,
verwendet das eigenst\"andige Programm \texttt{nisspec3}
\cite{NisspecHandbuch} parallel auf derselben Logdatei. Die Theorie und
das Format der Ausgabe sind dort vollst\"andig dokumentiert
(Maradudin-Bessel-Mehrphonon-Entwicklung, Pulvermittel,
ina32-Thermodynamik nach Achterhold/Parak/Sturhahn, mit allen Quellen).

Dieses Kapitel wurde bewusst kurz gehalten -- f\"ur die theoretischen
Grundlagen verweisen wir auf das nisspec3-Handbuch.
\end{infobox}

\section{Warum die Trennung sinnvoll ist}

\begin{itemize}
  \item \textbf{Klare Verantwortlichkeiten}: \texttt{modenanalyse\_2fe2s}
        analysiert die geometrischen und symmetrischen Eigenschaften
        einzelner Modes; \texttt{nisspec3} berechnet die spektroskopisch
        beobachtbare Antwort (NIS-Spektrum, pDOS, Thermodynamik).
  \item \textbf{Unabh\"angige Anwendbarkeit}: Mode-Klassifikation ist
        n\"utzlich auch ohne NIS (z.\,B.\ f\"ur reine PCET-Studien).
        Umgekehrt ist NIS-Synthese n\"utzlich auch ohne PCET-Score.
  \item \textbf{Bessere Wartbarkeit}: zwei kleine, fokussierte Pakete
        statt eines monolithischen.
  \item \textbf{Kompatibilit\"at}: \texttt{nisspec3} liest dieselben
        Gaussian-\texttt{.log}- und ORCA-\texttt{.hess}-Dateien; die
        Auswertungen sind direkt vergleichbar.
\end{itemize}

\section{Workflow zur kombinierten Auswertung}

Wenn Sie sowohl Mode-Klassifikation als auch NIS-Spektren wollen,
laufen zwei separate Programme parallel auf derselben Logdatei:
...
Die Ausgaben werden in unabh\"angige Verzeichnisse geschrieben und
...
Die folgende \"Ubersicht zeigt alle 53 Konfigurationsfelder mit ihren
...
\texttt{log\_file}      & \emph{(Pflicht)} & Pfad zur DFT-Frequenz-
  ...
  \texttt{freq=hpmodes}) und ORCA \texttt{.hess} (ab v3.3, automatisch
...
\texttt{freq\_windows}  & \texttt{None} & Liste von Fenstern wie
  ...
  pDOS werden dennoch global \"uber alle Modes berechnet (siehe
  \S\ref{sec:nis}). \\
\bottomrule
\end{longtable}
...
auch f\"ur die NIS- und pDOS-Auswertung. Da $^{57}$Fe-projizierte
Beitr\"age oberhalb $\sim$800 cm$^{-1}$ verschwindend klein sind und
NRVS-Experimente in dem Bereich messen, ist eine 800-cm$^{-1}$-Grenze
sowohl experimentell als auch numerisch sinnvoll. Vergleichbarkeit mit
``ina32''-Outputs bleibt gewahrt, sofern dort ebenfalls bei $\sim$800
...
\midrule
\endhead
\texttt{analyze\_nis}     & \texttt{True} & NIS-/pDOS-Analyse aktivieren. \\
\texttt{nis\_lineshape}   & \texttt{"gauss"} & Linienprofil:
  \texttt{"gauss"}, \texttt{"lorentz"} oder \texttt{"voigt"}. Voigt
  ist die NRVS-Standardwahl bei kombinierter instrumenteller (Gauss)
  und phononischer (Lorentz) Verbreiterung. \\
\texttt{nis\_fwhm\_gauss} & \texttt{8.07} & Gauss-FWHM in cm$^{-1}$
  (entspricht 1.0 meV instrumenteller Aufl\"osung). \\
\texttt{nis\_fwhm\_lorentz} & \texttt{0.0} & Lorentz-FWHM in cm$^{-1}$
  (Phononen-Lebenszeit-Verbreiterung; experimentell 0.5--2.0). \\
\texttt{nis\_n\_points}   & \texttt{2000}  & Frequenzgitter-Punkte
  zwischen \texttt{nis\_freq\_min} und \texttt{nis\_freq\_max}. \\
\texttt{nis\_freq\_min}, \texttt{nis\_freq\_max} & \texttt{None}     &
  Eigenes Frequenzgitter f\"ur NIS, falls von der Modenanalyse abweichend.
  \texttt{None} \"ubernimmt \texttt{freq\_min}/\texttt{freq\_max}. \\
\texttt{nis\_phonon\_order} & \texttt{1} & Maximaler Phononen-Order
  in der Maradudin-Entwicklung. 1 = nur 1-Phonon (Standard), 2 = mit
  Obert\"onen, 3 = mit Drei-Phonon (selten n\"otig, $O(N^3)$). \\
\texttt{nis\_n\_theta}    & \texttt{10}   & Theta-St\"utzstellen f\"ur das
  Pulvermittel. 10 ist der nisspec2-Default und meist ausreichend. \\
...
\paragraph{Veraltete v3.6-Parameter} (werden seit v3.7.3 mit Warnung
ignoriert, damit alte TOMLs nicht crashen): \texttt{pcet\_dr\_normalization\_a},
...
zur fr\"uheren Score-Konstruktion (siehe \S\ref{sec:reorg}).
...
Warnung ignoriert): \texttt{tsne\_perplexity}. t-SNE und PCA wurden
in v3.7.2 entfernt; siehe \S\ref{sec:embeddings}.
...
[INFO] PCET-Reorg: aktiv (2 His-Liganden, 4 H-Bond-Akzeptoren).
       ...
       HA im Reaktionskoordinaten-Modus (v3.7.1).
\end{lstlisting}
...
keine SS-Analyse, keine PDB-Liganden-Mapping. Das ist der einfachste
...
Pro Lauf entstehen drei Excel-Workbooks: das \emph{Hauptauswertungs-Excel}
mit allen modenbezogenen Sheets, das \emph{NIS-Excel} mit Spektrum und
Thermodynamik, sowie das \emph{interpolierte pDOS-Excel}. Hier die
\"Ubersicht.
...
\texttt{SS\_*} & Sekund\"arstruktur-Auswertung: pro Mode die
...
\texttt{NIS\_Spektrum} & Simuliertes NIS-Spektrum. Spalten: Energie
  in meV und cm$^{-1}$, Spektrum f\"ur jede konfigurierte Temperatur
  (\texttt{cfg.temp\_k\_nis}), getrennt in elastisch und inelastisch
  (wenn \texttt{nis\_split\_elastic = True}). Direkt nisspec3-konform
  \"uber den \texttt{nis}-Adapter berechnet. \\

\texttt{NIS\_Intensitaeten} & Diskrete Intensit\"aten (vor Verbreiterung)
  pro Mode mal Temperatur. Erlaubt die Identifikation einzelner
  Modes als Linien im NIS-Spektrum. \\

\texttt{Fe\_pDOS\_Spektrum} & Fe-projizierte pDOS auf dem
  NIS-Frequenzgitter, mit Lineshape verbreitert. \\

\texttt{Fe\_pDOS\_diskret} & Diskrete pDOS-Eintr\"age vor Verbreiterung
  (eine Linie pro Mode). \\

\texttt{Thermodynamik} & Tabelle der thermodynamischen Gr\"o\ss en pro
  Temperatur: $\fLM$, $\langle x^2_\Fei\rangle$ (1D),
  $\langle k_\Fei\rangle$, $c_V$, Entropie, mittlere Phononenenergie.
  Alle Werte mit expliziten 1D-pro-Fe-Annotationen
  (siehe \texttt{nisspec3}-Handbuch). \\

\texttt{Info} & Wie im Hauptauswertungs-Excel, aber NIS-spezifisch
...
\texttt{Interpoliert} & pDOS auf gleichm\"a\ss igem Gitter (Schrittweite
  \texttt{cfg.interp\_step}, Default 0.05 cm$^{-1}$), f\"ur direktes
  Plotten in Origin oder Gnuplot. \\
...
Plain-Text-Befund (\texttt{BEFUND.txt}) geschrieben. Inhalt:
...
\begin{itemize}
  ...
  \item PCET-/ET-Score-Aktivierung mit Diagnose
...
\texttt{config} & 684 & Konfigurations-Datenklasse \texttt{Config}
  ...
  Default-Werte. Backward-Compat: alte v3.6/v3.7.x-TOMLs werden mit
  Warnung statt Crash akzeptiert (\texttt{\_legacy\_silent\_drop}).
...
\texttt{reorganization} & 1096 & Marcus-Hush-Reorg-Pipeline (v3.7).
  ...
  FeS, NH, HA. HA im Reaktionskoordinaten-Modus (v3.7.1), ohne
  Gauss-Daempfung in der Aggregation (v3.7.4).
...
\texttt{pcet\_et} & 311 & Geometrie-Adapter fuer die
  ...
  Score-Berechnung wurde in v3.7.0 entfernt; der Marcus-Hush-Pivot
  ist im Modul \texttt{reorganization} implementiert. \\

\texttt{embedding} & 603 & UMAP-Embedding der 31-dim
  Feature-Matrix; HDBSCAN-Clusterung; SS-bezogene
  Embedding-Variante. PCA und t-SNE wurden in v3.7.2 entfernt
  (siehe \S\ref{sec:embeddings}). \\
...
\texttt{export} & 2063 & Excel-Schreiber fuer 25 Sheet-Typen,
  ...
  in v3.7.5 entfernt ($\sim 191$ Zeilen). \\
...
\midrule
\#1, \#4 & SCSD nach Kingsbury, rigorose Klassifikation
                                & \texttt{core} \\
...
\#10     & Multi-Cluster-Erkennung
                                & \texttt{geometry} \\
\#11     & HP/Standard-Frequenz-Konsistenzpr\"ufung
                                & \texttt{logio} \\
...
Charakter zeigt (alle Z-Scores nahe 0), ist er statistisch
unsicher und sollte ignoriert werden.
...
Tritt auf, wenn \texttt{modenanalyse\_2fe2s} \emph{allein} (ohne das
\texttt{nisspec3}-Wheel) installiert wird. Pip sucht nach
\texttt{nisspec3} auf PyPI und findet es nicht (es ist nur lokal als
...
Oder das mitgelieferte Setup-Skript: Doppelklick auf
\texttt{install.bat}, oder in PowerShell \texttt{.\textbackslash install.ps1}.

\subsubsection*{``Invalid wheel filename''}

PEP 427 verlangt Punkte in Versionsnummern. Wenn die Datei
...
\begin{enumerate}
  ...
  \item \textbf{Sekund\"arstruktur-Auto-Erkennung}: Da das PDB keine
        HELIX/SHEET-Records enth\"alt, wurden 3 SS-Elemente per
...
\begin{tabular}{p{1.5cm} p{6cm} p{4cm}}
  ...
  $w_l$           & pDOS-Gewicht des Atoms $l$ (in nisspec3) & dimensionslos \\
...
(elastischer Anteil im NRVS-Spektrum). Berechnung in nisspec3.
...
nicht das NRVS-Spektrum selbst berechnet (das macht nisspec3),
sondern die Reorg-Beitr\"age, die zur Bandenidentifikation
...
Enth\"alt 4 Sheets mit den UMAP-Projektionen und HDBSCAN-Clusterungen.

\subsubsection*{\texttt{Projektionen}}

Eine Zeile pro Mode, Spalten: \texttt{Modus \#}, \texttt{Frequenz},
...
C$_\alpha$-Bewegung. Verwendet f\"ur die Embedding-Feature-Matrix.

\subsubsection*{\texttt{UMAP\_Cluster}}

Eine Zeile pro Mode mit ihrer HDBSCAN-Cluster-ID
(\texttt{cluster\_label} -- $-1$ = Rauschen, $0, 1, \ldots$ =
Cluster-Index).

\subsubsection*{\texttt{UMAP\_Cluster\_Profil}}

Pro HDBSCAN-Cluster eine Zeile mit dem mittleren Feature-Vektor
des Clusters. Erlaubt die Antwort: \emph{Was charakterisiert
diesen Cluster im Feature-Raum?}
...
9--11 Sheets, je nach SS-Komplexit\"at:

\subsubsection*{Pro-SS-Element-Sheets (\texttt{SS\_*})}

Pro SS-Metrik ein eigenes Sheet:
...
Jedes Sheet hat eine Zeile pro Mode + Spalte pro SS-Element.

\subsubsection*{\texttt{SS\_UMAP\_Cluster} und \texttt{SS\_UMAP\_Profil}}

Wie \texttt{UMAP\_Cluster} und \texttt{UMAP\_Cluster\_Profil}, aber
auf der SS-Feature-Matrix (statt der Cluster-Liganden-Matrix). Findet
...
mitgeliefert mit dem nisspec3-Paket.
...
% --- v3.5: Methodische Quellen fuer Vibrationsanalyse -------------------
