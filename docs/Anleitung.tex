...
Diese Manual-Ausgabe beschreibt das Paket im Stand v1.2.3
...
\begin{itemize}\setlength{\itemsep}{0pt}
  ...
  \item \textbf{v1.2.0}: Kritik-Review-Release. Multi-Cluster-L\"aufe
        brechen nicht mehr beim ersten fehlschlagenden Cluster den ganzen
        Prozess ab; die SCSD-Symmetriezerlegung (die gegen das
        ver\"offentlichte \texttt{scsdpy} still ausgefallen war) ist
        wiederhergestellt; die PCET-Wasserstoffbr\"ucken-Erkennung erh\"alt
        ein D--H$\cdots$A-Winkelkriterium ($\geq 120^\circ$) und eine
        korrigierte Akzeptor-Abstandsgewichtung; die His-Protonierung ist
        auf das richtige Residuum beschr\"ankt; die Kabsch-Ausrichtung nutzt
        elementgematchte Anker mit RMSD-Akzeptanzschwelle; und die
        Massengewichtung verwendet die QM-eigenen Atommassen. Mehrere dieser
        Punkte \"andern das numerische Ergebnis, betroffene Systeme also
        einmal neu ausf\"uhren und die Kennzahlen neu validieren. Ein im
        Standardlauf ausgef\"uhrter Regressionstest fixiert
        $\Lambda_\text{FeFe}/\Lambda_\text{FeS}$ f\"ur das Referenzsystem.
  \item \textbf{v1.2.1}: Die reduzierten ORCA-Massen werden als
        $\mu_k = \sum_i m_i\,|l_{i,k}|^2$ rekonstruiert (verifiziert an
        realen ORCA-\texttt{.hess}-Dateien und \"ubereinstimmend mit der
        Gaussian-``Red.\ mass''-Konvention) und ersetzen einen
        \texttt{1.0}-amu-Platzhalter, der die absoluten ORCA-Amplituden um
        $\sqrt{\mu}$ zu gro\ss{} machte. Der Gaussian-Pfad bleibt
        unver\"andert.
  \item \textbf{v1.2.2}: Das Interpolationsraster von
        \texttt{*\_analysis\_interp\#\#.xlsx} und
        \texttt{*\_analysis\_SSE\_interp\#\#.xlsx} umfasst jetzt exakt den
        angeforderten Frequenzbereich, statt auf
        [erste Mode $-$ \texttt{interp\_edge\_extend},
        letzte Mode $+$ \texttt{interp\_edge\_extend}] beschnitten zu
        werden. Eine fehlende \texttt{freq\_min} bedeutet $0.0$ -- passend
        zum Ordnerlabel \texttt{0--800\_cm-1}. Rasterpunkte ohne
        benachbarte Mode werden mit $0.0$ belegt, sodass Raster
        verschiedener Fenster und L\"aufe spaltenweise vergleichbar sind;
        nur ein Lauf ohne angeforderten Bereich folgt weiterhin den Daten.
        Betroffen ist die Zahl der f\"uhrenden/abschlie\ss{}enden Zeilen,
        nicht die interpolierten Werte.
  \item \textbf{v1.2.3}: Robustheit der Excel-Exporte. Ist eine
        Zieldatei gesperrt (unter Windows: die Datei des vorigen Laufs
        ist noch in Excel ge\"offnet), wird die Arbeitsmappe als
        \texttt{<Name>\_1.xlsx} geschrieben und dies auf der Konsole
        \emph{und} im REPORT gemeldet -- vorher blieb still die alte
        Datei liegen. Im Multi-Window-Modus erreichen die Kontextmoden
        jetzt die Fenster- und Gesamtexporte (die Kontextgrenzen folgen
        der H\"ulle aller Fenster, nicht mehr den dort ignorierten
        \texttt{freq\_min}/\texttt{freq\_max}); die Warnung
        \texttt{no context modes available} und der unverankerte obere
        Rasterrand entfallen damit. Der REPORT nennt im Multi-Window-Modus
        das Basisverzeichnis, die Frequenzspalte der Interpolations-Excel
        ist auf sechs Nachkommastellen gerundet.
\end{itemize}
\end{infobox}
...
\texttt{interp\_boundary\_mode} & \texttt{"context"} & Verhalten am
  Fenster-Rand: \texttt{"context"} (Kontextmoden verankern beide
  R\"ander), \texttt{"zero"} (Randwert $0.0$ -- ``keine Daten in diesem
  Bereich''), \texttt{"nearest"} (Randwert = n\"achste vorhandene
  Mode). \\
\texttt{interp\_edge\_extend} & \texttt{0.5} & Raster-Puffer (cm$^{-1}$)
  \"uber die erste/letzte Mode hinaus. Seit v1.2.2 nur noch wirksam, wenn
  \emph{kein} Frequenzbereich angefordert wurde; mit Bereich umfasst das
  Raster exakt diesen Bereich. \\
