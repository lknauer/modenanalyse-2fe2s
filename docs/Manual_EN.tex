...
Because the eigenvectors are mass-weighted, $\alpha_{\OOP}$ is at
the same time the \emph{kinetic-energy fraction} of the mode
flowing perpendicular to the cluster plane -- a connection that is
relevant for the polarization dependence of NRVS spectra.
...
\begin{itemize}
  ...
  \item \emph{Secondary structure} (HELIX/SHEET fractions): handled
        ...
        \texttt{SS\_UMAP\_Cluster}). The mode UMAP and SS UMAP
...
\end{equation}
where $\alpha_X(i) \in [0, 1]$ is the contribution of mode to
bond $X$. Hence a pure stretching mode of the bond ($\alpha = 1$)
at $T \to 0$ has a reorganization contribution of $\hbar\omega/4$
(zero-point energy divided by 2).
...
\begin{itemize}
  \item \textbf{Modulation spectra} $M_X(\omega)$ -- Gaussian-broadened
        accumulation of the absolute modulations:
        \begin{equation}
          M_X(\omega)
          ...
          \label{eq:M-spectrum}
        \end{equation}
        Default: $\sigma_M = 5$\,cm$^{-1}$, configurable via
...
\texttt{analyze\_ss}    & \texttt{True} & Activate secondary-structure
  analysis (helices, sheets from PDB). \\
\texttt{ss\_chain}      & \texttt{""}   & Chain filter for
  SS detection. Empty = same as \texttt{pdb\_chain}. \\
\bottomrule
\end{longtable}
...
mode'': only atoms from the DFT run are evaluated, with no SS
...
\texttt{SS\_*} & Secondary-structure analysis: per mode the
  displacement amplitude in each helix or sheet
  (\texttt{SS\_Helix1}, \texttt{SS\_Sheet2}, \ldots). Allows
...
\texttt{SS\_UMAP\_Cluster}, \texttt{SS\_UMAP\_Profile} & As above,
...
After the run, open the \texttt{UMAP\_Cluster\_Profile} sheet. Per
...
to 0), it is statistically uncertain and should be ignored.
...
\begin{enumerate}
  ...
  \item \textbf{Secondary-structure auto-detection}: since the
        PDB contains no HELIX/SHEET records, 3 SS elements were
...
\paragraph{Secondary structure (SS).}
...
Contains 4 sheets with the UMAP projections and HDBSCAN
clusterings.

\subsubsection*{\texttt{Projections}}

One row per mode. Columns: \texttt{Mode \#}, \texttt{Frequency},
\texttt{UMAP\_x}, \texttt{UMAP\_y}. Directly plottable as a 2D
embedding.

\subsubsection*{\texttt{Ca\_amplitudes}}

Per mode and per ligand group: amplitude statistics of the
C$_\alpha$ motion. Used for the embedding feature matrix.

\subsubsection*{\texttt{UMAP\_Cluster}}

One row per mode with its HDBSCAN cluster ID
(\texttt{cluster\_label}: $-1$ = noise; $0, 1, \ldots$ = cluster
index).

\subsubsection*{\texttt{UMAP\_Cluster\_Profile}}

One row per HDBSCAN cluster with the cluster's mean feature
vector. Allows the question: \emph{what characterizes this
...
9--11 sheets, depending on SS complexity:

\subsubsection*{Per-SS-element sheets (\texttt{SS\_*})}

One sheet per SS metric:
\texttt{SS\_amplitude\_mean},
\texttt{SS\_amplitude\_max},
\texttt{SS\_com\_amplitude} (center-of-mass amplitude),
\texttt{SS\_lateral\_std} (lateral motion scatter),
\texttt{SS\_lateral\_amplitude},
\texttt{SS\_stretching} (axial length change),
\texttt{SS\_axial\_amplitude},
\texttt{SS\_tilting\_angle} (helix tilt),
\texttt{SS\_internal\_amplitude} (internal motion).

Each sheet has one row per mode and one column per SS element.

\subsubsection*{\texttt{SS\_UMAP\_Cluster} and \texttt{SS\_UMAP\_Profile}}

Like \texttt{UMAP\_Cluster} and \texttt{UMAP\_Cluster\_Profile},
but on the SS feature matrix (instead of the cluster-ligand
