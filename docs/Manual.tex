...
This release of the manual reflects the package at version v1.2.3
...
\begin{itemize}\setlength{\itemsep}{0pt}
  ...
  \item \textbf{v1.2.0}: critical-review release. Multi-cluster runs no
        longer abort the whole process on the first failing cluster; the
        SCSD symmetry decomposition (which had become a silent no-op against
        the published \texttt{scsdpy}) is restored; PCET hydrogen-bond
        detection gains a D--H$\cdots$A angle criterion ($\geq 120^\circ$)
        and a corrected acceptor-distance weighting; His protonation is
        restricted to the correct residue; Kabsch cluster alignment uses
        element-matched anchors with an RMSD acceptance gate; and mass
        weighting now uses the QM program's own atomic masses. Several of
        these change numerical output, so affected systems should be re-run
        and headline reorganization numbers re-validated. A default-run
        regression pins $\Lambda_\text{FeFe}/\Lambda_\text{FeS}$ for the
        reference fixture.
  \item \textbf{v1.2.1}: ORCA reduced masses are reconstructed as
        $\mu_k = \sum_i m_i\,|l_{i,k}|^2$ (verified against real ORCA
        \texttt{.hess} files and confirmed to match the Gaussian ``Red.\
        mass'' convention), replacing a \texttt{1.0}~amu placeholder that
        made absolute ORCA amplitudes too large by $\sqrt{\mu}$. The
        Gaussian path is unaffected.
  \item \textbf{v1.2.2}: the interpolation grid of
        \texttt{*\_analysis\_interp\#\#.xlsx} and
        \texttt{*\_analysis\_SSE\_interp\#\#.xlsx} now spans exactly the
        requested frequency range instead of being clipped to
        [first mode $-$ \texttt{interp\_edge\_extend},
        last mode $+$ \texttt{interp\_edge\_extend}]. A missing
        \texttt{freq\_min} means $0.0$, matching the
        \texttt{0--800\_cm-1} folder label. Grid points without a nearby
        mode are filled with $0.0$, so grids from different windows and
        runs are comparable column-by-column; only a run with no
        requested range still follows the data. Affects the number of
        leading/trailing rows, not the interpolated values.
  \item \textbf{v1.2.3}: export robustness. If an output file is locked
        (on Windows: the file of the previous run is still open in Excel)
        the workbook is written as \texttt{<name>\_1.xlsx} and this is
        reported on the console \emph{and} in the REPORT -- previously the
        stale file silently remained. In multi-window mode the context
        modes now reach the per-window and overall exports (the context
        ranges follow the hull of all windows instead of the
        \texttt{freq\_min}/\texttt{freq\_max} fields that are ignored
        there), which removes the spurious \texttt{no context modes
        available} warning and the un-anchored upper grid edge. The REPORT
        names the base directory in multi-window mode, and the frequency
        column of the interpolation Excel is rounded to six decimals.
\end{itemize}
\end{infobox}
...
\texttt{interp\_boundary\_mode} & \texttt{"context"} & Behavior at
  the window edge: \texttt{"context"} (context modes anchor both
  edges), \texttt{"zero"} (edge value $0.0$ -- ``no data in this
  range''), \texttt{"nearest"} (edge value = nearest present mode). \\
\texttt{interp\_edge\_extend} & \texttt{0.5} & Grid padding
  (cm$^{-1}$) beyond the first/last mode. Since v1.2.2 this applies
  \emph{only} when no frequency range is requested; with a range the
  grid spans exactly that range. \\
