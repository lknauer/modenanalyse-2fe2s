...
This release of the manual reflects the package at version v1.0.4
...
\begin{itemize}\setlength{\itemsep}{0pt}
  ...
  \item \textbf{v1.0.3}: new diagnostic outputs -- the
        ...
        additional UMAP embeddings (SS-UMAP and Ca-UMAP) target
  ...
  \item \textbf{v1.0.4}: audit-driven uncertainty propagation fix
        ...
        \texttt{s\_*}-columns for N-H stretches, SS-element
...
scaling, SS-element used a hard-coded constant in place of
...
SS-element \texttt{amplitude\_mean}, embedding coordinates --
...
\begin{itemize}\setlength{\itemsep}{2pt}
  ...
  \item \texttt{s\_amplitude\_mean} / \texttt{s\_axial\_amplitude} /
        ...
        \texttt{analyze\_ss\_element}: pre-v1.0.4 used a hard-coded
        ...
        SS-element entries that were marked ``significant'' under
...
\texttt{His\_HN.xlsx}, \texttt{SCSD.xlsx}, and the SS-Element
...
non-zero entry in the per-SS-element feature row (\texttt{ss}-dict
non-empty after \texttt{analyze\_all\_ss}). Useful for identifying
collective protein-skeleton modes that visit multiple secondary
structures simultaneously. The \texttt{*\_clusters} sheet lists
modes with their SS-UMAP coordinates and HDBSCAN cluster ID; the
\texttt{*\_profile} sheet gives the Z-score profile of the top-30
discriminating SS-elements per cluster.
...
\begin{itemize}\setlength{\itemsep}{0pt}
  \item \emph{Global UMAP} (\texttt{Projections}): which modes
        ...
        respectively.
  \item \emph{SS-UMAP} (\texttt{SS\_UMAP\_*}): which modes share
...
\texttt{\_embedding\_SS\_UMAP.png} (3 panels: frequency / mode type /
